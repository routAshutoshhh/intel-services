...
app.post('/stationary-shops', async (req, res) => {
    const { shop_name, owner_name, address, phone_number, email } = req.body;
    try {
        const newStationaryShop = await db.query(
...
app.delete('/res-orders/:id', async (req, res) => {
    const orderId = req.params.id;
    try {
        const deletedOrder = await db.query('DELETE FROM orders WHERE id = $1 RETURNING *', [orderId]);
...
app.delete('/order-items/:id', async (req, res) => {
    const purchaseId = req.params.id;
    try {
        const deletedOrderItem = await db.query('DELETE FROM stat_orders WHERE purchase_id = $1 RETURNING *', [purchaseId]);
...
app.delete('/order-tracking/:id', async (req, res) => {
    const trackingId = req.params.id;
    try {
        const deletedOrderTracking = await db.query('DELETE FROM order_tracking WHERE id = $1 RETURNING *', [trackingId]);
